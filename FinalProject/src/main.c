...
SDL_Texture* enlargedItemTexture = NULL; // 新增變量來存儲放大顯示的物品圖片
int characterAffinity = 40; // 初始好感度
bool itemDisplayedInCenter = false; // 是否在屏幕中央顯示物品
char currentItemDescription[256]; // 存儲當前物品的描述
SDL_Texture* descriptionTexture = NULL; // 存儲描述文字的紋理
...
void updateScene(SDL_Renderer* renderer, SDL_Texture** bgTexture, SDL_Texture** characterTexture, TTF_Font* font, SDL_Color textColor, SDL_Texture** textTexture, SDL_Texture** button1Texture, SDL_Texture** button2Texture, SDL_Texture** itemTexture, SDL_Texture** affinityTexture, SDL_Texture** enlargedItemTexture, SDL_Texture** descriptionTexture);
    ...
    while ((key = toml_key_in(dialogue_table, i++))) {
        ...
        if (tab) {
            ...
            for (int j = 0; j < toml_array_nelem(opt_array); j++) {
                ...
                if (toml_rtos(toml_raw_in(opt_tab, "inventory_add"), &inventory_add) == 0 && inventory_add) {
...
void updateScene(SDL_Renderer* renderer, SDL_Texture** bgTexture, SDL_Texture** characterTexture, TTF_Font* font, SDL_Color textColor, SDL_Texture** textTexture, SDL_Texture** button1Texture, SDL_Texture** button2Texture, SDL_Texture** itemTexture, SDL_Texture** affinityTexture, SDL_Texture** enlargedItemTexture, SDL_Texture** descriptionTexture) {
    ...
    *textTexture = renderText(renderer, dialogues[currentDialogueIndex].text, font, textColor);
    ...
    if (*enlargedItemTexture != NULL) {
        SDL_DestroyTexture(*enlargedItemTexture);
        *enlargedItemTexture = NULL;
    }
    itemDisplayedInCenter = false;
        ...
        if (itemIndex != -1) {
            *itemTexture = loadTexture(items[itemIndex].icon, renderer);
            if (*itemTexture == NULL) {
                ...
                itemRect.x = SCREEN_WIDTH * SCALE - 300 * SCALE;
                itemRect.y = 50 * SCALE;
                itemRect.w = 50 * SCALE;
                itemRect.h = 50 * SCALE;
                *enlargedItemTexture = loadTexture(items[itemIndex].icon, renderer);
                if (*enlargedItemTexture != NULL) {
                    itemDisplayedInCenter = true;
                    snprintf(currentItemDescription, sizeof(currentItemDescription), "%s", items[itemIndex].description);
                    *descriptionTexture = renderText(renderer, currentItemDescription, font, textColor);
                }
    ...
    if (characterAffinity <= 20) {
        *affinityTexture = loadTexture("example-game/assets/20%.png", renderer);
    } else if (characterAffinity <= 40) {
        *affinityTexture = loadTexture("example-game/assets/40%.png", renderer);
    } else if (characterAffinity <= 60) {
        *affinityTexture = loadTexture("example-game/assets/60%.png", renderer);
    } else if (characterAffinity <= 80) {
        *affinityTexture = loadTexture("example-game/assets/80%.png", renderer);
    } else {
        *affinityTexture = loadTexture("example-game/assets/100%.png", renderer);
    }
...
int runGame1() {
    ...
    // 讀取劇本
    loadDialoguesAndCharacters("example-game/script.toml");
    ...
    if (characterIndex != -1) {
        ...
        playerSurface = IMG_Load("example-game/assets/player_happy.png"); // 預設角色圖片
    }
    ...
    // 加載字型
    TTF_Font* font = TTF_OpenFont("example-game/assets/NotoSansTC-Regular.ttf", 50 * SCALE);
    if (font == NULL) {
    ...
    while (!quit) {
        ...
        while (SDL_PollEvent(&e) != 0) {
            if (e.type == SDL_QUIT) {
                ...
                if (itemDisplayedInCenter && enlargedItemTexture != NULL) {
                    // 檢查點擊是否在放大物品上
                    if (x >= (SCREEN_WIDTH * SCALE / 2 - 100 * SCALE) && x <= (SCREEN_WIDTH * SCALE / 2 + 100 * SCALE) &&
                        y >= (SCREEN_HEIGHT * SCALE / 2 - 100 * SCALE) && y <= (SCREEN_HEIGHT * SCALE / 2 + 100 * SCALE)) {
                        itemDisplayedInCenter = false; // 物品被點擊後不再居中顯示
                    }
                } 
                else {
                    if (isMouseOverButton(x, y, button1Rect)) {
                        ...
                        if (nextDialogueIndex != -1) {
                            ...
                            updateScene(renderer, &bgTexture, &characterTexture, font, textColor, &textTexture, &button1Texture, &button2Texture, &itemTexture, &affinityTexture, &enlargedItemTexture, &descriptionTexture);
        ...
        if (itemDisplayedInCenter && enlargedItemTexture != NULL) {
            ...
            SDL_Rect blackRect = {0, 0, SCREEN_WIDTH * SCALE, SCREEN_HEIGHT * SCALE};
            SDL_RenderFillRect(renderer, &blackRect);

            // 在屏幕中央顯示放大物品
            SDL_Rect enlargedItemRect = {SCREEN_WIDTH * SCALE / 2 - 100 * SCALE, SCREEN_HEIGHT * SCALE / 2 - 100 * SCALE, 200 * SCALE, 200 * SCALE};
            SDL_RenderCopy(renderer, enlargedItemTexture, NULL, &enlargedItemRect);

            // 顯示物品描述
            if (descriptionTexture != NULL) {
                SDL_Rect descriptionRect = {SCREEN_WIDTH * SCALE / 2 - 150 * SCALE, SCREEN_HEIGHT * SCALE / 2 + 120 * SCALE, 300 * SCALE, 50 * SCALE};
